...
--
-- TODO:
--   should probably check for a response of @Fail@ from either commands
--   or queries.
--
--------------------------------------------------------------------------------
...
import Control.Monad (guard, join, liftM, when)
...
-- | Open a connection to the Minecraft server or return @Nothing@.
--
openMCPI ::
    Bool  -- ^ Set to @True@ to get debugging messages printed to @stderr@.
    -> MaybeT IO ConnInfo
openMCPI flag = do
    as <- lift $ getAddrInfo Nothing Nothing (Just mcPort)
    let a = head as -- note: getAddrInfo never returns an empty list
    sock <- lift $ socket (addrFamily a) Stream defaultProtocol
    lift $ setSocketOption sock KeepAlive 1

    let ehdl :: IOException -> IO Bool
        ehdl _ = return False
       
    flag <- lift ((connect sock (addrAddress a) >> return True)
                  `CE.catch` ehdl)
    guard flag

    h <- lift $ socketToHandle sock ReadWriteMode
...
--   printed to @stderr@.
--
--   @Nothing@ is returned if the connection to MineCraft PI
--   can not be made.
--
runMCPI' :: Bool -> MCPI a -> IO (Maybe a)
runMCPI' flag p = runMaybeT $ do
  conn <- openMCPI flag

  let -- ehdl :: IOException -> IO (Maybe a)
      ehdl _ = closeMCPI conn >> return Nothing

  ans <- lift ((Just `liftM` runReaderT p conn) `CE.catch` ehdl)
  return ans

{-
  bracket
    (runMaybeT (openMCPI flag))
    closeMCPI
    (runReaderT p)
-}
